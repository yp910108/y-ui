...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map((week) => t(`y.datepicker.weeks.${week}`)),
    dayNames: weeks.map((week) => t(`y.datepicker.weeks.${week}`)),
    monthNamesShort: months.map((month) => t(`y.datepicker.months.${month}`)),
    monthNames: months.map((month, index) => t(`y.datepicker.month${index + 1}`)),
    amPm: ['am', 'pm']
  }
}
