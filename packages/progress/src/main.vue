<template>
  ...
  >
    ...
    <div v-if="showText && !textInside" class="y-progress__text" :style="{ fontSize: progressTextSize + 'px' }">
  ...
  computed: {
    ...
    iconClass() {
      if (this.status === 'warning') {
        return 'y-icon-warning'
      }
      if (this.type === 'line') {
        return this.status === 'success' ? 'y-icon-circle-check' : 'y-icon-circle-close'
      } else {
        return this.status === 'success' ? 'y-icon-check' : 'y-icon-close'
      }
    },
    progressTextSize() {
