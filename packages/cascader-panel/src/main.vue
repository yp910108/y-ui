<template>
  <div :class="['y-cascader-panel', border && 'is-bordered']" @keydown="handleKeyDown">
    <cascader-menu ref="menu" v-for="(menu, index) in menus" :index="index" :key="index" :nodes="menu" />
  </div>
</template>
...
import { noop, coerceTruthyValueToArray, isEqual, isEmpty, valueEquals } from 'main/utils/util'
...
const getSibling = (el, distance) => {
  const { parentNode } = el
  if (parentNode) {
    const siblings = parentNode.querySelectorAll('.y-cascader-node[tabindex="-1"]')
  ...
  methods: {
    ...
    handleKeyDown(e) {
      ...
      switch (keyCode) {
        ...
        case KeyCode.left:
          ...
          if (preMenu) {
            const expandedNode = preMenu.$el.querySelector('.y-cascader-node[aria-expanded="true"]')
        ...
        case KeyCode.right:
          ...
          if (nextMenu) {
            const firstNode = nextMenu.$el.querySelector('.y-cascader-node[tabindex="-1"]')
            focusNode(firstNode)
          }
          break
        case KeyCode.enter:
    ...
    scrollIntoView() {
      ...
      menus.forEach((menu) => {
        ...
        if (menuElement) {
          const container = menuElement.querySelector('.y-scrollbar__wrap')
          const activeNode =
            menuElement.querySelector('.y-cascader-node.is-active') ||
            menuElement.querySelector('.y-cascader-node.in-active-path')
