...
import { isEqual } from 'main/utils/util'
...
export default {
  ...
  methods: {
    ...
    renderCheckbox() {
      ...
      return <y-checkbox value={node.checked} indeterminate={node.indeterminate} disabled={isDisabled} {...events} />
    },
    ...
    renderCheckIcon() {
      return <i class="y-icon-check y-cascader-node__prefix" />
    },
    renderLoadingIcon() {
      return <i class="y-icon-loading y-cascader-node__postfix" />
    },
    renderExpandIcon() {
      return <i class="y-icon-arrow-right y-cascader-node__postfix" />
    },
    renderContent() {
      const { panel, node } = this
      const render = panel.renderLabelFn
      const vnode = render ? render({ node, data: node.data }) : null
      return <span class="y-cascader-node__label">{vnode || node.label}</span>
    }
  },
  computed: {
    config() {
