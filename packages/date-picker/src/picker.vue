<template>
  ...
  >
    <i slot="prefix" class="y-input__icon" :class="triggerClass" @click="handleFocus" />
    <i
      v-if="haveTrigger"
      slot="suffix"
      class="y-input__icon"
  ...
  <div
    ...
    ref="reference"
    v-clickoutside="handleClose"
    class="y-date-editor y-range-editor y-input__inner"
    :class="[
      'y-date-editor--' + type,
      pickerSize ? `y-range-editor--${pickerSize}` : '',
  ...
  >
    <i :class="['y-input__icon', 'y-range__icon', triggerClass]" />
    <input
      v-bind="firstInputId"
      class="y-range-input"
      autocomplete="off"
    ...
    <slot name="range-separator">
      <span class="y-range-separator">{{ rangeSeparator }}</span>
    </slot>
    <input
    ...
    <i
      v-if="haveTrigger"
      class="y-input__icon y-range__close-icon"
      :class="[showClose ? '' + clearIcon : '']"
      @click="handleClickIcon"
    />
  </div>
...
<script>
import Vue from 'vue'
import { formatDate, parseDate, isDateObject, getWeekNumber } from 'main/utils/date-util'
...
// year/month/monthrange/week/date/daterange/datetime/datetimerange/dates  time/time-select/timerange
...
const newPopper = {
  ...
  data() {
    return merge({ visibleArrow: true }, VuePopper.data)
  },
...
export default {
  ...
  computed: {
    ...
    triggerClass() {
      return this.prefixIcon || (this.type.indexOf('time') !== -1 ? 'y-icon-time' : 'y-icon-date')
    },
    selectionMode() {
      if (this.type === 'week') {
        return 'week'
