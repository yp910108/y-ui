<template>
  ...
  >
    <tbody>
      <tr>
        <th v-if="showWeekNumber">{{ t('y.datepicker.week') }}</th>
        <th v-for="(week, key) in WEEKS" :key="key">{{ t('y.datepicker.weeks.' + week) }}</th>
      </tr>
      <tr class="y-date-table__row" v-for="(row, key) in rows" :class="{ current: isWeekActive(row[1]) }" :key="key">
...
import { arrayFindIndex, arrayFind, coerceTruthyValueToArray } from 'main/utils/util'
