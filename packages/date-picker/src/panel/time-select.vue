<template>
  <transition name="y-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      ref="popper"
      :style="{ width: width + 'px' }"
      :class="popperClass"
      class="y-picker-panel time-select y-popper"
    >
      <y-scrollbar noresize wrap-class="y-picker-panel__content">
        <div
          v-for="item in items"
...
export default {
  ...
  methods: {
    ...
    scrollToOption(selector = '.selected') {
      const menu = this.$refs.popper.querySelector('.y-picker-panel__content')
