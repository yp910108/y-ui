...
import YButtonGroup from 'packages/button-group'
...
export default {
  ...
  methods: {
    ...
    initAria() {
      ...
      if (!this.splitButton) {
        ...
        this.triggerElm.setAttribute('class', (this.triggerElm.getAttribute('class') || '') + ' y-dropdown-selfdefine') // 控制
      }
    },
    initEvent() {
  ...
  watch: {
    visible(val) {
      this.broadcast('YDropdownMenu', 'visible', val)
      this.$emit('visible-change', val)
    },
    focusing(val) {
      const selfDefine = this.$el.querySelector('.y-dropdown-selfdefine')
      if (selfDefine) {
        // 自定义
        if (val) {
