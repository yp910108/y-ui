<template>
  <span>
    <transition :name="transition" @after-enter="handleAfterEnter" @after-leave="handleAfterLeave">
      <div
        class="y-popover y-popper"
        :class="[popperClass, content && 'y-popover--plain']"
        ref="popper"
    ...
    <span class="y-popover__reference-wrapper" ref="wrapper">
...
export default {
  ...
  mounted() {
    ...
    if (this.trigger === 'click') {
      ...
      if (this.tabindex < 0) {
        console.warn('[Yui Warn][Popover]a negative taindex means that the element cannot be focused by tab key')
      }
